...
public class ID3TagUtils {
    ...
    public static boolean setID3Tags(File sourceFile, ID3Tags id3Tags, boolean clearOriginal) {
        ...
        if (clearOriginal) {
            ...
            try {
                oID3V2_3_0Tag = (ID3V2_3_0Tag) oMediaFile.getID3V2Tag(); //获取MP3文件的ID3V2的标签
            } catch (ID3Exception e) {
                e.printStackTrace();
            }
            if (oID3V2_3_0Tag == null) {
                oID3V2_3_0Tag = new ID3V2_3_0Tag(); //D3V2为空创建一个新标签
            }
        }

        try {
            id3Tags.fillID3Tag(oID3V2_3_0Tag); //填充D3V2标签内容
            oMediaFile.setID3Tag(oID3V2_3_0Tag); //给MP3设置D3V2标签内容
            oMediaFile.sync();
            return true;
        } catch (ID3Exception e) {
            e.printStackTrace();
